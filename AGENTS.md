- # AGENTS.md — Cognitive Platform (NLCS)

  This file is read by Codex at the start of every session. Follow every convention here unless Ben explicitly overrides it in the current session.
  ...
  var sprintEndDateById = iterations.SelectMany(iteration => iteration.IterationIds
                                                					  .Select(id => (id, iteration.EndDate)))
      						    .ToDictionary(sprintInfo => sprintInfo.id, sprintInfo => sprintInfo.EndDate);
  ...
  **Do not test (out of scope for unit tests):**
  ...
  6. Update the appropriate file(s) in the `C:\Users\benho\source\Application Documentation\The CP Universe\_Repo Docs\As-Built` folder. The `INDEX.md` file has an overview of the files in this folder.  If an appropriate file does not exist, please create a new one and update `INDEX.md` so it knows about this new file.

  ------

  ## Soft Delete Invariant

  Every domain object has `IsDeleted` / `DeletedUtc`. Hard deletes are never performed (with the exception of the Admin app). Tests that exercise delete behavior must assert soft-delete semantics, not hard removal.
...
* Other files with future work:
  ...
  * C:\Users\benho\source\Application Documentation\The CP Universe\_Repo Docs\To concider-Ideas
  * C:\Users\benho\source\Application Documentation\The CP Universe\Natural Language Command System\Bug Log - Tech Debt - Dev.md
...
Read this file for what **"Discipline Mode"** is: C:\Users\benho\source\Application Documentation\The CP Universe\Natural Language Command System\Discipline Mode Guidelines for AGENTS.md
